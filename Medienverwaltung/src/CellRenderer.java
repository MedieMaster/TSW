...
public class CellRenderer extends JLabel implements ListCellRenderer<Medium>{
	...
	public Component getListCellRendererComponent(JList<? extends Medium> list, Medium value, int index,
		...
		+ "<br>Datum/Uhrzeit: " + value.getErstellungsDatumZeit() + "<br>");
		...
		if(value instanceof Film) {
			Film f = (Film)value;
			text.append("<HTML><Body>Person: " + f.getDauer()
		...
		if(value instanceof Video) {
			Video v = (Video)value;
			text.append("<HTML><Body>Person: " + v.getDauer()
		...
		if(value instanceof Musiktitel) {
			...
			text.append("<HTML><Body>Künstler: " + m.getKuenstler()
			+ "<br>Dauer: " + m.getDauer()
